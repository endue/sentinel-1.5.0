...
public class ContextUtil {
     ...
     * 存储Context name对应的EntranceNode，key是Context name默认为"default_context_name"
     */
    ...
    /**
     * 从该方法中可以看出：
     * 1. 每个线程都持有一个属于自己的Context，不存在时则创建存储在全局属性contextHolder中
     *  1.1 Context上线问中存储了资源对应的EntranceNode
     * 2. 每个资源都对应一个属于自己的DEntranceNode，不存在时则创建存储在全局属性contextNameNodeMap
     *  2.1  EntranceNode中存储了资源名称以及类型为EntryType.IN
     * 3. 相同名称的Context指向同一个EntranceNode
     *
     * @param name 非显式调用ContextUtil#enter方法时，该值默认为default_context_name
     * @param origin
     * @return
     */
    protected static Context trueEnter(String name, String origin) {
        ...
        if (context == null) {
            ...
            // 相同context name获取相同的EntranceNode
                    ...
                    try {
                        ...
                        if (node == null) {
                            if (contextNameNodeMap.size() > Constants.MAX_CONTEXT_NAME_SIZE) {
                                ...
                                // 更加上下文名称构建一个入口节点EntranceNode，添加到ROOT中
